...
public class P002122 {
	...
	private static void alinea2(PrintStream out) {
		...
		while (sc.hasNextLine()) {
			...
			if (firstChar == '#') {
				Client c = em.addClient(fields[0], fields[1]);
				e = em.addEvent(c, LocalDate.parse(fields[2]));
			} else if (firstChar == '*') {
				String type = fields[0];
				String modality = fields[1];
				int participants = Integer.parseInt(fields[2]);

				if (type.equals("Sport")) {
					e.addActivity(new Sport(Sport.Modality.valueOf(modality), participants));
				} else if (type.equals("Catering")) {
					e.addActivity(new Catering(Catering.Option.valueOf(modality), participants));
				} else if (type.equals("Culture")) {
					e.addActivity(new Culture(Culture.Option.valueOf(modality), participants));
				}
			}
