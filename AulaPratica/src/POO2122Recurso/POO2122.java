...
public class POO2122 {
	...
	private static void alinea2(PrintStream out) {
		out.println("\nAlínea 2) ----------------------------------\n");
		AgenciaTuristica at2 = new AgenciaTuristica( "SempreAMexer", "Porto" ) ; ///////////// modificar////////////////////
		// Adicione a seguir o código necessário para a leitura do ficheiro 
			try{
				Scanner sc = new Scanner(new FileReader("agencia.txt"));

				while(sc.hasNextLine()){
					String line = sc.nextLine();
					String keyWord = line.substring(0, 1);
					String[] args = line.substring(2).split(",");
					
					switch(keyWord){
						case "P":
							at2.pacoteTuristico(args[0], Integer.parseInt(args[1]), Integer.parseInt(args[2]));
							break;
						case "R":
							at2.reserva(at2.getPacoteTuristico(args[0]), Integer.parseInt(args[1]));
							break;
					}
				}
				sc.close();
